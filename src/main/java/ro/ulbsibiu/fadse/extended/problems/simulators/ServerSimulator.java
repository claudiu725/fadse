...
public class ServerSimulator extends SimulatorWrapper {
    ...
    public void performSimulation(Individual individual) {
        ...
        if (neighbors == null || neighbors.size() < 1) {
        	logger.error("No neighbors configured");
            return;
        }
        ...
        while (individualsToSend.size() > 0) {
            ...
            if (!ind.isFeasible()) {
            	logger.error("ERROR: Individual is not feasible!");
            }
...
//                        Logger.getLogger(ServerSimulator.class.getName()).log(Level.INFO, "Found an available client... " + n.toString());
                    ...
                    logger.info("Individual sent to: " + n);
                } catch (UnknownHostException ex) {
                	logger.error("Don't know about host", ex);
                } catch (IOException ex) {
                    logger.error("Couldn't get I/O for the connection or ACK not received from" + n.getIp() + ":" + n.getPort(), ex);
                } catch (Exception ex) {
...
//                    Logger.getLogger(ServerSimulator.class.getName()).log(Level.INFO, "Neighbor: " + n.getIp()+":"+n.getPort() + " has all the slots full. Trying to sleep for 200 ms before trying the next neighbor.");
    ...
    public void join() {
        ...
        logger.info("ServerSimulator.join - all the simulations are done");
        ...
        List<Individual> duplicateDetector = new ArrayList<Individual>();
        List<Message> cleanMessages = new ArrayList<Message>();
            ...
            for (Message localKeptMessage : simulationStatus.getSentMessages()) {
                if (receivedMessage.getMessageId().equals(localKeptMessage.getMessageId())) {
                    ...
                    if (duplicateDetector.contains(localKeptMessage.getIndividual())) {
                        ...
                        logger.info("We already have results for individual with benchmark: "+localKeptMessage.getIndividual().getBenchmark()+" should we copy its results?");
                    } else {
        ...
        logger.info("Clean messages size: " + cleanMessages.size());
        ...
        for (Message localKeptMessage : cleanMessages) {
            ...
            try {
                if (objs.size() != environment.getInputDocument().getObjectives().size()) {
                    ...
                    logger.error("individual has not all the objectives filled");
                    infeasible = true;
                }
            } catch (Exception e) {
            	logger.error("Something wrong with the objectives: " + e.getMessage());
            }
            for (int i = 0; i < objs.size(); i++) {
                ...
                if (o.getValue() == 0) {
                	logger.error("individual has objectives set to 0 - marking him as infeasible[1]");
        ...
        for (Message localkeptMessage : cleanMessages) {
            ...
            try {
                if (localkeptMessage.getIndividual().getObjectives().size() != environment.getInputDocument().getObjectives().size()) {
                    ...
                    logger.error("individual has not all the objectives filled[1]");
                    infeasible = true;
                }
                ...
                for (Objective o : localkeptMessage.getIndividual().getObjectives()) {
                    if (o.getValue() == 0) {
                        logger.error("individual has objectives set to 0 - marking him as infeasible[2]");
                ...
                if (localKeptSolution.getCounter() != environment.getInputDocument().getBenchmarks().size() * environment.getInputDocument().getObjectives().values().size()) {
                    logger.error("individual does not have results for all the benchmarks, or has more results (" + (localKeptSolution.getCounter() + "!=" + environment.getInputDocument().getBenchmarks().size() * environment.getInputDocument().getObjectives().values().size()) + ") : ");
                    for (int i = 0; i < environment.getInputDocument().getObjectives().values().size(); i++) {
                        logger.error(localKeptSolution.getSum(i));
                    }
                    infeasible = true;
//TODO - TEST if the objectives are set in this method or calling set bad values is already too late
        ...
        logger.info("Join method finished.");
    ...
    private void redistributeUnfinishedSimulations() {
    	logger.info("redistributeUnfinishedSimulations called...");
        ...
        while (simulationStatus.getNumberOfActiveSimulations() > 0) {
            ...
            try {
            	logger.info("Still waiting for " + simulationStatus.getActiveSimulations() + " results");
                try {
                ...
                logger.error("redistributeUnfinishedSimulations[1]: " + ex.getMessage());
            }
        }
        logger.info("redistributeUnfinishedSimulations finished.");
    }
...
//            Logger.getLogger(ServerSimulator.class.getName()).log(Level.INFO, "handling messageID " + messageId + "...");
            ...
            if (s != null && System.currentTimeMillis() - s.getSimulationStartedTime().getTime() > 1000 * 60 * maxTime) {
            	...
            	logger.info("Retries for this message: " + s.getRetries());
                if (s.getRetries() > 1) {
                    s.getMessage().getIndividual().markAsInfeasibleAndSetBadValuesForObjectives("Too many retries");
                    s.setActive(false);
                    logger.info("Individual has been marked as infeasible.");
                } else {
                ...
                try {
                    ...
                    logger.error("detectAndRescheduleCrashedClients[1]", ex);
                } catch (IOException ex) {
                    logger.error("detectAndRescheduleCrashedClients[2]", ex);
                } catch (Exception ex) {
                    logger.error("detectAndRescheduleCrashedClients[3]", ex);
                }
            }
        }
